  ...
  return (
    <div className="w-full space-y-6 select-none">
      ...
      <div className="rounded-3xl bg-slate-900/90 border border-white/[0.08] p-4 sm:p-6 shadow-2xl backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base sm:text-lg font-black text-white flex items-center gap-2">
              Croquis Arquitectónico del Bulevar • 50 Mesas (500 Sillas)
            </h3>
          </div>
        ...
        <div className="flex items-center gap-3 text-xs flex-wrap justify-center bg-slate-950/80 px-4 py-2.5 rounded-2xl border border-slate-800">
          <span className="flex items-center gap-1.5 text-emerald-300 font-semibold">
            <span className="h-3 w-3 sm:h-3.5 sm:w-3.5 rounded-full bg-emerald-500 ring-2 ring-emerald-500/30 shadow-sm" />
            Libre
          </span>
          <span className="flex items-center gap-1.5 text-amber-300 font-black">
            <span className="h-3 w-3 sm:h-3.5 sm:w-3.5 rounded-full bg-amber-400 ring-2 ring-amber-400/50 shadow-md animate-pulse" />
            Tu Selección ({selectedSeats.length})
          </span>
          <span className="flex items-center gap-1.5 text-slate-400">
            <span className="h-3 w-3 sm:h-3.5 sm:w-3.5 rounded-full bg-slate-800 border border-slate-700" />
            Ocupada
          </span>
        </div>
      </div>

      {/* Sector Quick Filter Horizontal Chips (Mobile Ergonomics) */}
      <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 no-scrollbar select-none">
        <button
          type="button"
          onClick={() => setActiveSector('ALL')}
          className={`px-3.5 py-2 rounded-xl text-xs font-black transition-all shrink-0 whitespace-nowrap active:scale-95 ${
            activeSector === 'ALL'
        ...
        {sectors.map((sec) => (
          <button
            ...
            className={`px-3 py-2 rounded-xl text-xs font-black transition-all shrink-0 whitespace-nowrap active:scale-95 ${
              activeSector === sec
                ? 'bg-amber-400 text-slate-950 shadow-md shadow-amber-400/20'
                : 'bg-slate-900 border border-slate-800 text-slate-300 hover:text-white'
            }`}
      ...
      <div className="relative rounded-3xl border-2 border-amber-500/30 bg-slate-950 shadow-2xl overflow-hidden">
        ...
        <div className="relative w-full bg-gradient-to-b from-blue-950 via-slate-900 to-slate-950 p-4 sm:p-6 text-center border-b-4 border-amber-400/80 shadow-2xl">
          ...
          <div className="relative z-10 max-w-lg mx-auto">
            <div className="inline-flex items-center gap-2 rounded-full bg-blue-950/80 border border-sky-400/40 px-3 sm:px-4 py-1 text-[11px] sm:text-xs font-black text-sky-200 mb-1.5">
              <Music className="h-3.5 w-3.5 text-amber-400 animate-bounce" />
        ...
        <div className="relative flex">
          ...
          <div className="w-7 sm:w-16 shrink-0 bg-gradient-to-r from-black via-slate-950 to-slate-900 border-r-2 sm:border-r-4 border-black relative flex flex-col justify-between items-center py-4 sm:py-6 select-none shadow-2xl">
            ...
            <div className="flex flex-col justify-around h-full space-y-16 sm:space-y-24 z-10">
              {[1, 2, 3, 4, 5, 6].map(post => (
                <div key={post} className="flex flex-col items-center group">
                  <div className="h-2.5 w-2.5 sm:h-3.5 sm:w-3.5 rounded-full bg-amber-400 shadow-[0_0_14px_rgba(245,158,11,0.9)] animate-pulse border border-amber-200" />
                  <div className="w-1.5 sm:w-2 h-8 sm:h-12 bg-black rounded-sm mt-0.5 border-x border-slate-700 shadow-md" />
                </div>
              ))}
            </div>

            {/* Vertical Railing Label (Desktop Only) */}
            <div className="hidden sm:block absolute top-1/2 -translate-y-1/2 -rotate-90 text-[10px] font-mono font-black tracking-widest text-slate-400 uppercase whitespace-nowrap pointer-events-none">
              BARANDA NEGRA PERIMETRAL
            </div>
          </div>

          {/* 2. CENTER: THE 50 BANQUET TABLES ON THE BOULEVARD WALKWAY */}
          <div className="flex-1 p-2 sm:p-8 bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:20px_20px] sm:[background-size:24px_24px] space-y-6 sm:space-y-10 min-w-0">
            {sectors.map((sec) => {
              if (activeSector !== 'ALL' && activeSector !== sec) return null;
              const sectorTables = tables.filter(t => t.sector === sec);

              return (
                <div key={sec} className="rounded-2xl sm:rounded-3xl border border-white/[0.08] bg-slate-950/90 p-3 sm:p-6 shadow-2xl backdrop-blur-md">
                  <div className="flex items-center justify-between border-b border-slate-800/80 pb-2 sm:pb-3 mb-4 sm:mb-6">
                    <h4 className="text-xs sm:text-sm font-black uppercase text-amber-400 tracking-wider flex items-center gap-1.5 sm:gap-2 truncate">
                      <Sparkles className="h-3.5 w-3.5 text-amber-400 shrink-0" />
                      <span className="truncate">{sectorNames[sec]}</span>
                    </h4>
                    <span className="text-[10px] sm:text-[11px] text-slate-400 font-bold bg-slate-900 px-2.5 py-0.5 sm:py-1 rounded-full border border-slate-800 font-mono shrink-0 ml-2">
                      10 mesas
                    </span>
                  </div>

                  {/* 10 Realistic Banquet Tables Grid */}
                    ...
                    {sectorTables.map((table) => {
                      ...
                      return (
                        <div
                          ...
                          className={`group relative rounded-2xl sm:rounded-3xl p-2 sm:p-4 text-center transition-all duration-150 border-2 cursor-pointer flex flex-col items-center justify-between shadow-xl min-w-0 ${
                        ...
                        >
                          ...
                          <div className="relative w-28 h-28 sm:w-32 sm:h-32 my-1 flex items-center justify-center shrink-0">
                            ...
                            {table.seats.map((seat, i) => {
                              ...
                              const radius = 46; // px exactos desde el centro para ajuste perfecto
                              ...
                              return (
                                <div
                                  ...
                                  className={`absolute h-3 w-3 sm:h-3.5 sm:w-3.5 rounded-full border shadow-sm transition-transform duration-150 ${
                                    isSelected
                            ...
                            <div
                              className={`w-16 h-16 sm:w-20 sm:h-20 rounded-full flex flex-col items-center justify-center text-center shadow-xl border-2 sm:border-4 transition-all z-10 ${
                                tableSelectedCount > 0
                            ...
                            >
                              ...
                              <span className="text-[8px] sm:text-[9px] font-black tracking-tight leading-none text-sky-200 uppercase">
                                MESA
                              </span>
                              <span className="text-xs sm:text-base font-black text-amber-300 leading-tight">
                                {table.id}
                              </span>
                              <span className="text-[6px] sm:text-[8px] font-bold text-slate-300 mt-0.5">
                                10 Sillas
                              </span>
                            </div>
                          </div>
          ...
          <div className="w-7 sm:w-16 shrink-0 bg-gradient-to-l from-slate-950 via-slate-800 to-slate-900 border-l-2 sm:border-l-4 border-slate-600 relative flex flex-col justify-between items-center py-4 sm:py-6 select-none shadow-2xl">
            ...
            <div className="flex flex-col justify-around h-full space-y-16 sm:space-y-24 z-10">
              {[1, 2, 3, 4, 5, 6].map(block => (
                <div key={block} className="flex flex-col items-center">
                  <div className="w-3.5 sm:w-5 h-6 sm:h-8 bg-slate-700 rounded-sm border border-slate-500 shadow-md" />
                  <div className="h-1.5 w-1.5 sm:h-2 sm:w-2 rounded-full bg-emerald-500/70 mt-1" title="Jardines del Seminario" />
                </div>
              ))}
            </div>

            {/* Vertical Muro Label (Desktop Only) */}
            <div className="hidden sm:block absolute top-1/2 -translate-y-1/2 rotate-90 text-[10px] font-mono font-black tracking-widest text-slate-300 uppercase whitespace-nowrap pointer-events-none">
        ...
        <div className="relative w-full bg-slate-950 border-t-2 sm:border-t-4 border-slate-800">
          
          {/* Visual Pavement & Curving Walkway Architecture */}
          <div className="relative flex items-stretch h-32 sm:h-44 overflow-hidden">
            
            {/* Bottom-Left Curved Black Railing */}
            <div className="w-7 sm:w-16 bg-black border-r-2 sm:border-r-4 border-b-2 sm:border-b-4 border-black relative flex items-end justify-center pb-3 sm:pb-4 rounded-bl-3xl">
              <div className="h-3 w-3 sm:h-4 sm:w-4 rounded-full bg-amber-400 shadow-[0_0_16px_rgba(245,158,11,1)] animate-pulse" />
            </div>

            {/* Center-to-Right Curving Paved Boulevard Floor */}
            <div className="flex-1 bg-gradient-to-r from-slate-950 via-slate-900 to-amber-950/60 p-3 sm:p-6 flex items-center justify-between border-b-2 sm:border-b-4 border-black relative overflow-hidden min-w-0">
              ...
              <div className="relative z-10 flex items-center gap-2 sm:gap-3.5 min-w-0">
                <div className="h-10 w-10 sm:h-14 sm:w-14 rounded-2xl bg-amber-500/20 text-amber-400 border-2 border-amber-500/40 flex items-center justify-center shrink-0 shadow-lg animate-pulse">
                  <CornerDownRight className="h-5 w-5 sm:h-8 sm:w-8" />
                </div>
                <div className="min-w-0">
                  <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-300 font-mono text-[9px] sm:text-[10px] font-black uppercase border border-amber-500/30 mb-0.5 sm:mb-1">
                    <span>GIRO DEL BULEVAR</span>
                  </div>
                  <h4 className="text-xs sm:text-xl font-black text-white leading-tight truncate">
                    El Bulevar gira hacia la derecha ➔
                  </h4>
                  <p className="text-[10px] sm:text-xs text-slate-400 mt-0.5 line-clamp-1 sm:line-clamp-none">
                    Sector E: donde la caminería dobla y concluye el trazado.
                  </p>
                </div>
              </div>

              {/* Right End Terminal Boundary */}
              <div className="relative z-10 flex flex-col items-end text-right shrink-0 bg-slate-950/90 border border-amber-500/30 px-2.5 sm:px-4 py-2 sm:py-3 rounded-2xl shadow-xl ml-2">
                <span className="text-[9px] sm:text-[10px] font-mono font-bold text-amber-400 uppercase tracking-wider block">
                  LÍMITE
                </span>
                <span className="text-[10px] sm:text-xs font-black text-white block mt-0.5 whitespace-nowrap">
                  Fin del Paseo
                </span>
              </div>
            </div>
            ...
            <div className="w-7 sm:w-16 bg-slate-800 border-l-2 sm:border-l-4 border-slate-600 relative flex items-start justify-center pt-3 sm:pt-4">
              <div className="w-3.5 sm:w-5 h-6 sm:h-8 bg-slate-700 rounded-sm border border-slate-500 shadow-md" />
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/90 p-0 sm:p-4 backdrop-blur-xl animate-fadeIn">
          <div className="relative w-full max-w-xl max-h-[92vh] overflow-y-auto rounded-t-3xl sm:rounded-3xl border-t-2 sm:border-2 border-amber-500/60 bg-slate-900 p-4 sm:p-7 shadow-2xl text-center">
            ...
            <button
              ...
              className="absolute top-4 right-4 rounded-full bg-slate-800 p-2 text-slate-300 hover:text-white hover:bg-slate-700 transition-all active:scale-90 z-30"
              aria-label="Cerrar modal"
            >
              <X className="h-5 w-5" />
            </button>

            <div className="inline-flex items-center gap-1.5 rounded-full bg-amber-500/20 border border-amber-500/40 px-3 py-0.5 text-xs font-black text-amber-300 mb-1.5">
            ...
            <p className="text-xs text-slate-300 mt-1 font-medium max-w-md mx-auto">
              👇 Toca las sillas verdes para reservarlas o pulsa <strong>"Seleccionar Mesa Completa"</strong>:
            </p>

            {/* Circular Banquet Table Graphic with 10 Chairs */}
            <div className="w-full flex items-center justify-center overflow-hidden py-2 sm:py-3">
              <div className="relative w-[280px] h-[280px] sm:w-[320px] sm:h-[320px] flex items-center justify-center select-none origin-center shrink-0">
                
                {/* Central Round Table Surface */}
                <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full bg-gradient-to-br from-blue-950 via-slate-900 to-blue-900 border-4 border-amber-400 shadow-2xl flex flex-col items-center justify-center text-white z-10 pointer-events-none">
                  <span className="text-[8px] sm:text-[9px] uppercase font-bold text-sky-300 tracking-wider">Bulevar</span>
                  <span className="text-xl sm:text-3xl font-black text-amber-300">{activeTableModal.id}</span>
                  <span className="text-[8px] font-bold text-slate-300 mt-0.5">10 Comensales</span>
                </div>

                {/* 10 Interactive Physical Chairs (100% Concéntricas y Táctiles) */}
                {activeTableModal.seats.map((seat, i) => {
                  const angle = (i * 360) / 10 - 90;
                  const radius = 104; // px exactos optimizados para móvil y escritorio
                  const rad = (angle * Math.PI) / 180;
                  const x = Math.round(radius * Math.cos(rad));
                  const y = Math.round(radius * Math.sin(rad));
                  ...
                  return (
                    <button
                      ...
                      className={`absolute h-10 w-10 sm:h-12 sm:w-12 rounded-full flex flex-col items-center justify-center transition-transform duration-150 active:scale-90 cursor-pointer ${
                        isSelected
                    ...
                    >
                      {isSelected ? (
                        ...
                        <Armchair className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
                      )}
                      <span className="text-[9px] sm:text-[10px] font-black leading-none mt-0.5">{seat.number}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="mt-2 sm:mt-3 pt-2 sm:pt-3 border-t border-slate-800 text-left">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-xs text-amber-400 hover:text-amber-300 font-bold underline active:scale-95"
                >
                  ⚡ Toda la Mesa (10 Sillas)
                </button>
              </div>
                ...
                {activeTableModal.seats.map((seat) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isTaken}
                      onClick={() => toggleSeat(activeTableModal, seat.number)}
                      className={`p-1.5 sm:p-2 rounded-xl text-xs font-bold transition-all active:scale-95 flex flex-col items-center justify-center gap-0.5 ${
                        isSelected
                    ...
                    >
                      <span className="text-[8px] opacity-75">Silla</span>
